...
| 2.2.0 | Import-count fix (round 1). Every per-mapping `controlIdentifier` in the 7 RegScale export envelopes is now zero-padded to match the DoW catalog's `controlId` format exactly (`AC-1` → `AC-01`, `AC-2(1)` → `AC-02(01)`, `CM-3(6)` → `CM-03(06)`). Previously the envelopes shipped the unpadded OSCAL-native form. Padding alone did not resolve the 257-count drop — see v2.3.0 for the real root cause. Affected files (7): `fedramp_rev5_high-regscale-export.json` (351 identifiers re-padded), `dod_ccsrg_il4_moderate-regscale-export.json` (282), `dod_ccsrg_il4_high-regscale-export.json` (359), `dod_ccsrg_il5_nss-regscale-export.json` (506), `dod_ccsrg_il6_nss-regscale-export.json` (535), `dod_il5_nss_manifest-regscale-export.json` (506), `dod-ccsrg-regscale-export.json` (2,188). OSCAL artifacts under `profiles/archive/` are untouched. |
| **2.3.0** | **Live-tenant catalog retarget. Replaced the static DoW catalog (`catalogs/cnssi-1253-dow-rev5-catalog.json`) with the live RegScale tenant export of "DoW — NIST 800-53 — Rev 5 (CNSSI 1253)" (catalog `id=24`, uuid `476dbabf-f394-4375-8ddd-0a13f34c2f82`, 1,014 controls). Same catalog content; different tenant-side integer ids. Re-pointed all 7 RegScale export envelopes to this live catalog: every per-mapping `catalogId` flipped from `219` to `24` (1,815 changes across the 6 per-baseline files + 2,522 in the combined export = 4,337 catalogId rewrites), and every `controlId` integer remapped to the live tenant range (10756–11769) — e.g. AC-01: 1015 → 10756, SR-12: 2028 → 11769. UUIDs (`controlGuid`) and padded `controlIdentifier` values are unchanged because they're stable across catalog instances. Root cause of the FedRAMP 257-of-410 import drop: the previous envelopes referenced `catalogId=219`, which didn't exist in the user's tenant. The importer fell back to identifier-based resolution against whatever catalogs were currently visible (IL4 Moderate id=7 + IL4 High id=6), which only carry 257 of FedRAMP's 410 controls; the other 153 (mostly CP/SI/AU/SC enhancements unique to FedRAMP High) silently dropped. After v2.3.0, 100% of mappings across all 7 envelopes resolve cleanly against the live catalog by all three keys (`controlGuid` ↔ `uuid`, `controlIdentifier` ↔ `controlId`, `controlId` int ↔ `id`). Control counts unchanged (335 / 419 / 578 / 612 / 578 / 410, combined 2,522).** |
