...
| **2.2.0** | **Import-count fix. Every per-mapping `controlIdentifier` in the 7 RegScale export envelopes is now **zero-padded** to match the DoW catalog's `controlId` format exactly (`AC-1` → `AC-01`, `AC-2(1)` → `AC-02(01)`, `CM-3(6)` → `CM-03(06)`). Previously the envelopes shipped the unpadded OSCAL-native form, which caused RegScale's importer to silently drop mappings whose identifier didn't match the catalog's padded `controlId` — most visibly, the FedRAMP High profile imported as only 257 of its 410 controls. After this fix, 100% of mappings across all 7 envelopes resolve cleanly against the DoW catalog by all three keys (`controlIdentifier` ↔ `controlId`, `controlGuid` ↔ `uuid`, `controlId` int ↔ `id`). Control counts unchanged (335 / 419 / 578 / 612 / 578 / 410, combined 2,522). Affected files (7): `fedramp_rev5_high-regscale-export.json` (351 identifiers re-padded), `dod_ccsrg_il4_moderate-regscale-export.json` (282), `dod_ccsrg_il4_high-regscale-export.json` (359), `dod_ccsrg_il5_nss-regscale-export.json` (506), `dod_ccsrg_il6_nss-regscale-export.json` (535), `dod_il5_nss_manifest-regscale-export.json` (506), `dod-ccsrg-regscale-export.json` (2,188). Only the `controlIdentifier` string format changed; all other per-mapping fields (`controlId` int, `controlGuid`, `controlTitle`, catalog refs) and profile envelope metadata are unchanged. OSCAL artifacts under `profiles/archive/` are untouched.** |
